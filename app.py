...
def analyze_urls(dataframe, llm_client, model, language):
  ...
  # Crawl URLs
  adv.crawl(dataframe['url'], temp_file_path, follow_links=False)
  crawl_df = pd.read_json(temp_file_path, lines=True)
  open('crawl_file.jl', 'w').close()

  # Clean up strings in the DataFrame
  columns_to_clean = ['title', 'meta_desc', 'h1', 'body_text']
  ...
  df = pd.merge(dataframe, crawl_df[["url", "title", "meta_desc", "h1", "body_text"]], on=["url"])
  ...
  for index, row in df.iterrows():
      ...
      serp_analysis_result = analyze_serp_results(llm_client, model, combined_text, language)
      
      # Debug: Print the SERP analysis result
      print(f"SERP Analysis Result for URL {url}:\n{serp_analysis_result}\n")
      
      # Append the SERP analysis result to the results list
      results.append({
          "url": url,
          "page_type": serp_analysis_result.get("page_type"),
          "business_model": serp_analysis_result.get("business_model")
      })
  ...
  # Debug: Print the analysis DataFrame
